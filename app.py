...
Speed-optimised: streaming display, table-aware rendering, no duplicate translate calls
"""
...
.msg-ai-body th{background:#1a2744;color:#93c5fd;padding:10px 14px;text-align:left;
    font-weight:700;font-size:12px;border:1px solid #2d4a70;white-space:nowrap;}
.msg-ai-body td{padding:9px 14px;border:1px solid #1f2937;color:#d4d4d8;vertical-align:top;}
.msg-ai-body tr:nth-child(even) td{background:#0f1118;}
.msg-ai-body tr:hover td{background:#1a2133;}
/* Confidence banners */
...
/* Table badge (when tables found in answer) */
.table-badge{display:inline-flex;align-items:center;gap:5px;background:#0d1f3c;
    border:1px solid #1e3a5f;border-radius:6px;padding:4px 10px;
    font-size:11px;color:#60a5fa;font-weight:600;margin-bottom:10px;margin-left:40px;}
/* Sources */
...
/* Loading */
.loading-wrap{background:#0c0c10;border:1px solid #18181b;border-radius:14px;
...
/* Translation box */
.translation-box{background:#071510;border:1px solid #065f46;border-left:3px solid #34d399;
    border-radius:0 10px 10px 10px;padding:16px 20px;margin:14px 0 0 40px;
    font-size:15px;line-height:1.9;color:#86efac;}
.translation-label{font-size:11px;font-weight:700;letter-spacing:1px;color:#34d399;
...
.chips-label{font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;
    color:#3f3f46;margin-bottom:16px;}
.topbar-avatar{width:28px;height:28px;border-radius:8px;
    background:linear-gradient(135deg,#3b82f6,#8b5cf6);
...
# TOP BAR
# ══════════════════════════════════════════════════════
c_logo, c_gap, c_nc, c_tr, c_pp, c_menu, c_user_col = st.columns([3, 2, 1.2, 1.2, 1.2, 1.2, 1.5])
...
def _table_badge(count: int) -> str:
    if count == 0:
        return ""
    label = f"{'📊'} {count} table{'s' if count > 1 else ''} found in answer"
    return f'<div class="table-badge">{label}</div>'
...
def _render_chart(metadata: dict[str, Any]) -> None:
    ...
    with st.expander("📊 Relevance scores", expanded=False):
        names  = [
            f"DOC {d['index']}: {d['source'][:22]}…" if len(d['source']) > 22
            else f"DOC {d['index']}: {d['source']}"
            for d in docs
        ]
...
def _count_tables_in_answer(content: str) -> int:
    """Count markdown tables in the answer (lines with pipes)."""
    import re
...
def render_message(role: str, content: str, metadata: dict[str, Any]) -> None:
    ...
    # Table badge if answer contains rendered tables
    table_rows = _count_tables_in_answer(content)
    if table_rows > 2:   # at least one real table (header + sep + 1 row)
        st.markdown(_table_badge(max(1, table_rows // 3)), unsafe_allow_html=True)
    ...
    st.markdown(content)   # Streamlit renders markdown tables natively
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if metadata.get("translation_applied") and metadata.get("translated_answer"):
        lang = metadata.get("language_out", st.session_state.get("translate_lang", "none"))
        heading = {
            "hindi":   "🔤 हिंदी अनुवाद",
            "marathi": "🔤 मराठी अनुवाद",
        }.get(lang, "🔤 Translation")
        st.markdown(
            f'<div class="translation-box">'
            f'<div class="translation-label">{heading}</div>'
...
def _parse_stream(user_query: str, lang_out: str) -> tuple[str, dict]:
    """
    Consume the CRAG stream.
    - Status tokens → update spinner
    - Answer tokens  → render live via st.write_stream compatible approach
    - META token     → parse and return
    Returns (full_answer, meta_dict)
    """
    ...
    try:
        for raw_token in stream_crag_pipeline(user_query, language_out=lang_out):
            ...
            # ── First real token: clear spinner, open streaming display ───
            if not answer_started:
                answer_started = True
                status_ph.empty()
            ...
            # Live render incrementally (re-render full accumulated text)
            # Use answer_ph so we don't create new elements each token
            current = "".join(full_answer_parts)
            answer_ph.markdown(
                f'<div class="msg-ai-body" style="padding-left:40px;">'
                f'</div>',
                unsafe_allow_html=True,
            )
            answer_ph.markdown(current)

    except Exception as exc:
        status_ph.empty()
        full_answer_parts = [f"⚠️ Pipeline error: {exc}"]

    # Clear the live placeholder — final render happens via render_message
    status_ph.empty()
    answer_ph.empty()
...
if ask_btn and query.strip():
    ...
    # ── Translation (post-stream, non-blocking display) ───────────────────
    translated_answer   = ""
    ...
    if answer and lang != "none":
        try:
            ...
            translation_applied = bool(
                translated_answer and translated_answer.strip() != answer.strip()
            )
        except Exception:
            pass
    ...
    # Final render (replaces live placeholder)
